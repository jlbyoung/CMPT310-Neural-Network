...
class DenseLayer(BaseLayer):
    ...
    def compute_activation(self, x):
        ...
        output = np.dot(self.get_input_data(), self.w) + self.b
    ...
    def compute_gradient(self):
        ...
        """
        ##dE/do is output error gradient
        # YOUR CODE STARTS HERE (2)
        self.dw = np.dot(np.transpose(self.get_input_data()), self._output_error_gradient)
        ##self.dw = np.transpose(self.dw)
        mSize = np.shape(self._output_error_gradient)

        ones = np.ones((1, mSize[0]), dtype=int)
        self.db = np.dot(ones, self._output_error_gradient)
